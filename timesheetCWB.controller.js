sap.ui
		.define(
				...
				function(Controller, mApp, mTempo, mTexts, mTimecard, mTcDetails, tempoUtils, _ui,_d) {
					...
					return Controller
							.extend(
									"z_tpo_timecard.controller.timesheetCWB",
									{
										grid: null,
										enterTimeDialog : {
											Model : null,
										...
										onInit : function() {
											...
											sap.ui.getCore().getEventBus().subscribe("framework", "renderCWB", this.evOnRenderCWB, this);
										 ...
										 */
										onExit : function() {
											sap.ui.getCore().getEventBus().unsubscribe("framework", "renderCWB", this.evOnRenderCWB, this);
										...
										renderDetailsGrid : function() {
											...
											this.grid.addColumn(new sap.m.Column(
													{
														header :  new sap.m.Label({
										                	text : ((_ui.isCompact()) ? "W Beg" : "Wrk Per Start"),
														}),
														width: "20%",
													}));
											...
											this.grid.addColumn(new sap.m.Column(
													{
														header :  new sap.m.Label({
										                	text : ((_ui.isCompact()) ? "W End" : "Wrk Per End")
														}),
														width: "20%",
													}));
											...
											this.grid.addColumn(new sap.m.Column(
													{
														header :  new sap.m.Label({
										                	text : ((_ui.isCompact()) ? "W Dur" : "Wrk Per Duration")
														}),
													}));
											...
											_timerows.addCell(new sap.m.Text({
												text: {
													path : "ZzWrkPrBeg",
													formatter : function(d) {
														return self.formatDateTime(_d.timeToEST(d));
													}
												},
												visible: {
													path : "ZzAbsenceDay",
										...
										launchCWB: function(workdate){
											sap.ui.getCore().getEventBus().publish("tempoNav", "navToCWB", { "workdate": workdate, "pernr": mTempo.grid.EmployeeID, 'editable':mTempo.grid.editable});
